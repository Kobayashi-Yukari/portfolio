...
<tr>
@if (Auth::user()->flag == 0)
<td><input class="radio" type="radio" name="contact_id" value="{{ $contact->id }}"></td>
@endif
<td>
...
<br>
== 問い合わせ ==<br>
<div class="date" style="float: right;">[{{ $contact->created_at->format('Y-m-d') }}]</div>
{!! nl2br(e($contact->body)) !!}<br>
@if ($contact->reply != null)
== 返信 ==<br>
<div class="date" style="float: right;">[{{ $contact->updated_at->format('Y-m-d') }}]</div>
